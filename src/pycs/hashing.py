...
It's essential to note that for Python or for that matter any language, the hash table implementation needs to map
any given key to a unique numeric which shall act as an index into **a finite array within the memory bounds.**
Which means, a numeric hash function like (or their variations) - FNV, MurmurHash, CityHash, SipHash etc. are used
internally; typically these functions will produce a large number which again needs to be mapped to a smaller number
such that it can be an index into finite array of buckets. This is done using the modulo operator.
...
of the above-mentioned numeric hash functions to generate a unique number for the string. Even for use defined classes
